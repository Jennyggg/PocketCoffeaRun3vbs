...
def setup_dmatrix(input_df, norm_table,table_absweight_to_weight, label_column="process", weight_column="weight",test_size=0.2, random_state=43):
...
def training_bdt(dtrain, dval,features, num_round = 10, channel='whad_withbveto_mu',outdir='bdt',fold=0):
    params = {
    ...
    bdt = xgb.train(params, dtrain, num_boost_round=num_round, evals=evallist, evals_result=evals_result, maximize=False, early_stopping_rounds=5)
...
def test_bdt(models, dtest, dtrain, channel, df_test,df_train, tag="",unitary=True,outdir = "bdt",test_size=0.2):
    """
...
def balance_signal_background_weights_arrays(y, w, balance_to="background"):
    """
    Rescale weights so total signal weight equals total background weight.
    Returns a new weight array.
    """
    ...
    sum_signal = np.sum(w[signal_mask])
    sum_background = np.sum(w[background_mask])

    if sum_signal == 0 or sum_background == 0:
        print("Warning: one class has zero total weight.")
        return w

    if balance_to == "background":
        scale = sum_signal / sum_background
        w[signal_mask] *= scale
        print(f"Scaled signal weights by {scale:.3f} to match background total.")
    elif balance_to == "signal":
        scale = sum_signal / sum_background
        w[background_mask_mask] *= scale
        print(f"Scaled background weights by {scale:.3f} to match signal total.")
    else:
        raise ValueError("balance_to must be either 'signal' or 'background'.")

    # Scale background to match signal (or vice versa)
    scale = sum_signal / sum_background
    w[background_mask] *= scale
...
def main():
    ...
    if args.evaluate_only and args.model is not None:
        _,_,dtest,_, _, idx_test, df_fixed, df_unit = setup_dmatrix(args.out,norm,factor_absweight_to_weight, test_size=1.0, random_state=43)
        ...
        dtrain, dtest, idx_train, idx_test, df_fixed, df_unit = setup_dmatrix(args.out,norm,factor_absweight_to_weight,test_size = args.testsize)
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(X_trainval,y_trainval)):   
            ...
            bdt, evals_result = training_bdt(dtrain_fold, dval_fold,feature_names, num_round = 1000, channel=args.category,outdir = args.outdir,fold=fold)
            models.append(bdt)
            plot_training_curves(evals_result, outpath=f"{args.outdir}/training_curves_{args.category}_fold{fold}.png")
            plot_feature_importance(bdt,outpath=f"{args.outdir}/feature_importance_{args.category}_fold{fold}.png")
    ...
    auc1, acc1, hist_data1 = test_bdt(models, dtest_pure, dtrain_pure, args.category,df_test_unit, df_train_unit, tag="unitary", unitary=True,outdir = args.outdir,test_size=test_size)
    ### NORM TO XSEC

    auc2, acc2, hist_data2 = test_bdt(models, dtest, dtrain, args.category,df_test_xs, df_train_xs, tag="xsec", unitary=False, outdir = args.outdir,test_size=test_size)
