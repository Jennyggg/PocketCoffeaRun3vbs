...
class VBSSemileptonicProcessor(BaseProcessorABC):
    ...
    def fill_histograms_extra(self, variation):
        """Save per-category sumw_noqvgcal and sumw_qvg_variations.
        Only runs for the nominal shape variation; QvG weight variations are
        only meaningful when jet kinematics are at nominal.
        """
        if not self._isMC or variation != "nominal":
            return
        inv = ak.to_numpy(self.events.sf_qvg_central_inv)
        ...
        for category, cat_mask in self._categories.get_masks():
            ...
            # sumw_noqvgcal: nominal weight with sf_qvg divided out
            w_no_sf  = w_full * inv
            sumw_val = float(ak.sum(w_no_sf * mask_on_events))
            (self.output["sumw_noqvgcal"][category]
             .setdefault(self._dataset, {})
             .setdefault(self._sample, {}))["nominal"] = sumw_val
