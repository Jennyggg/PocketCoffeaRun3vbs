...
class VBSSemileptonicProcessor(BaseProcessorABC):
    ...
    def apply_object_preselection(self, variation):
       ...
       # ev["LeptonWithFakes"] = lep_with_fakes[ak.argsort(lep_with_fakes.pt, ascending=False)]
        ...
        ev["candidate_boost_veto"] = ev.FatJetGood[(_tau21(ev.FatJetGood) < 0.45) & (ev.FatJetGood.msoftdrop < 250) & (ev.FatJetGood.msoftdrop > 40) & (ev.FatJetGood.pt > 180)]

        dR_jets_jet = ev.JetGood.metric_table(ev.candidate_boost_veto)
        ...
        ev["candidate_boost"] = ev.FatJetGood[(_tau21(ev.FatJetGood) < 0.45) & (ev.FatJetGood.msoftdrop < 250) & (ev.FatJetGood.msoftdrop > 40) & (ev.FatJetGood.pt > 200) & (abs(ev.FatJetGood.eta) < 2.4)]
        ...
        ev["nFatJetCandidateVeto"] = ak.num(ev.candidate_boost_veto)
        ...
        if hasattr(self.params, 'classifiers'):
            ...
            for region in ["boosted_mu","boosted_e","resolved_mu","resolved_e"]:
                model_path = self.params.classifiers[self._year][region]
                models[region] = xgb.XGBClassifier()
                models[region].load_model(model_path)
                features[region] = models[region].get_booster().feature_names
                arrays_to_stack = []
                for name in features[region]:
                ...
                y_pred = models[region].get_booster().inplace_predict(X_test)
                ev[f"bdt_{region}"] = y_pred
