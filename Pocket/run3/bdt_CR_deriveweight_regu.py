...
config = {
    ...
    "output": "/afs/cern.ch/work/j/jinw/public/semilep_vbs_base/Pocket/run3/bdt_CR_reweight_resolved_lep_WCR_regu",
    "calibration": "/afs/cern.ch/work/j/jinw/public/semilep_vbs_base/Pocket/run3/bdt_CR_reweight_resolved_lep_WCR_regu/bdt_CR_reweight.py",
}
...
def compute_rw_table(h_data, h_mc,
                     smoothing_strength=1.0,
                     clip_min=0.1, clip_max=10.0):
    """
    For each (pt, eta) cell derive a tag-dependent SF using regularised
    least-squares instead of a raw bin-by-bin ratio.

    Both data and MC are normalised to unit area first (shape-only correction).
    The system solved per cell is:
        SF(t) * mc_norm(t) = data_norm(t)   for each tag bin t

    This is a diagonal system, but raw bin-by-bin division is noisy in sparse
    bins. Regularisation penalises large differences between adjacent SF values,
    producing a smooth SF(tag) curve while still fitting the data faithfully
    in well-populated bins.

    smoothing_strength : controls how strongly smoothness is enforced.
        0   -> raw ratio (no regularisation, original behaviour)
        1   -> moderate smoothing (recommended starting point)
        10+ -> very smooth, approaches a flat SF
    """
    n_tag = h_data.shape[2]
    rw    = np.ones_like(h_data, dtype=float)

    # smoothness regularisation matrix: penalise |SF(t+1) - SF(t)|
    # shape (n_tag-1, n_tag)
    n_smooth = n_tag - 1
    R = np.zeros((n_smooth, n_tag))
...
h_mc_cen   = make_rw_table(
    *get_region(pt_mc,   eta_mc,   tag_mc,   wgt_mc,   0.0, 2.5, 30.0, 450.0),
    pt_edges_central, eta_edges_central, tag_cal_edges)
rw_table_cen = compute_rw_table(h_data_cen, h_mc_cen, smoothing_strength=0.1)
...
h_mc_fwd   = make_rw_table(
    *get_region(pt_mc,   eta_mc,   tag_mc,   wgt_mc,   2.5, 4.7, 50.0, 450.0),
    pt_edges_forward, eta_edges_forward, tag_cal_edges)
rw_table_fwd = compute_rw_table(h_data_fwd, h_mc_fwd, smoothing_strength=0.1)


def plot_sf_vs_tag(rw_table, name, pt_edges_r, eta_edges_r):
    """Plot regularised SF(tag) for a few representative (pt, eta) cells."""
    ...
    cmap = plt.cm.viridis(np.linspace(0, 1, len(pt_idxs) * len(eta_idxs)))
    c = 0
    for pi in pt_idxs:
        for ei in eta_idxs:
            ...
            ax.plot(tag_cal_centres, rw_table[pi, ei, :],
                    color=cmap[c], linewidth=1.5, label=lbl)
            c += 1

    ax.axhline(1.0, color="black", linewidth=0.8, linestyle=":")
    ax.set_xlabel("QvG tag value")
    ax.set_ylabel("SF(tag)")
    ax.set_title(f"Regularised SF vs tag: {name}")
...
plot_sf_vs_tag(rw_table_cen, "central", pt_edges_central, eta_edges_central)
plot_sf_vs_tag(rw_table_fwd, "forward", pt_edges_forward, eta_edges_forward)
print("Saved SF vs tag diagnostic plots.")
