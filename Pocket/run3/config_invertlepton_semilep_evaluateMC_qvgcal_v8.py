...
import workflow_invertlepton_evaluate_qvgcal_WvsQCDcal_v8, custom_cut_functions
from workflow_invertlepton_evaluate_qvgcal_WvsQCDcal_v8 import VBSSemileptonicProcessor
...
cloudpickle.register_pickle_by_value(workflow_invertlepton_evaluate_qvgcal_WvsQCDcal_v8)
...
cfg = Configurator(
    ...
    weights_classes=common_weights+[SF_ele_trigger, MuonGoodLeadWeight,ElectronGoodLeadWeight],
    ...
    variables={
        ...
        "fj_WvsQCD":   HistConf([Axis(coll="candidate_boost1", field="particleNetWithMass_WvsQCD", bins=40, start=0, stop=1,   label=r"J^{W} WvsQCD")]),
        "fj_ZvsQCD":   HistConf([Axis(coll="candidate_boost1", field="particleNetWithMass_ZvsQCD", bins=40, start=0, stop=1,   label=r"J^{W} ZvsQCD")]),
        ...
        "bdt_resolved_mu_2D":       HistConf([
            Axis(coll="events", field="bdt_resolved_mu", bins=_RESOLVED_BINS, label="BDT mu resolved"),
        ...
        "bdt_resolved_e_2D":       HistConf([
            Axis(coll="events", field="bdt_resolved_e", bins=_RESOLVED_BINS, label="BDT e resolved"),
            Axis(coll="events", field="bdt_resolved_WW_WZ", bins=_RESOLVED_WWvsWZ_BINS, label="BDT resolved WW vs WZ")]),

        "bdt_boosted_mu":       HistConf([Axis(coll="events", field="bdt_boosted_mu", bins=_BOOSTED_BINS, label="BDT mu boosted")]),
        "bdt_resolved_mu":       HistConf([Axis(coll="events", field="bdt_resolved_mu", bins=_RESOLVED_BINS, label="BDT mu resolved")]),
        "bdt_boosted_e":       HistConf([Axis(coll="events", field="bdt_boosted_e", bins=_BOOSTED_BINS, label="BDT e boosted")]),
