...
class VBSSemileptonicProcessor(BaseProcessorABC):
    ...
    def apply_object_preselection(self, variation):
        ...
        mask_muon_medium = (
            (ev.Muon.pt>26) & (np.abs(ev.Muon.eta)<2.4) & (ev.Muon.pfRelIso04_all<0.2) & (ev.Muon.mediumId)
        )

        # The inverted "good" muons are those passing the loose selection but not the tight/
        ev["MuonGood"] = ev.Muon[mask_muon_medium & ~mask_muon_tight ]
        mask_ele_tight = (
        ...
        ev["ElectronGood"] = ev.Electron[mask_ele_medium & ~mask_ele_tight ]
