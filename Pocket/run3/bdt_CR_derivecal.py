...
config = {
    "categories": ["resolved_mu_WCR"],
    "year": "2022_postEE",
    "paths": ["/eos/user/j/jinw/VBS_forML/output_VBS_data2022postEE_deepmet_resolution_PTLLStitch_JetStitch_jet50_updatesel/"],
    "paths_nonprompt": ["/eos/user/j/jinw/VBS_forML/output_VBS_data2022postEE_deepmet_resolution_PTLLStitch_JetStitch_jet50_updatesel_invertlepton/"],
    "normtables": ["/afs/cern.ch/work/j/jinw/public/semilep_vbs_base/Pocket/run3/output_VBS_data2022postEE_save_CR"],
    "normtables_nonprompt": ["/afs/cern.ch/work/j/jinw/public/semilep_vbs_base/Pocket/run3/output_VBS_data2022postEE_nonprompt_save_CR"],
    "output": "/afs/cern.ch/work/j/jinw/public/semilep_vbs_base/Pocket/run3/bdt_CR_cal_resolved_mu_WCR",
...
# ─────────────────────────────────────────────────────────────────────────────
# 2. Build calibration from jet3 + jet4 stacked, in bins of (pt, |eta|)
# ─────────────────────────────────────────────────────────────────────────────
...
# jets used to derive the calibration (indices 1, 2, 3 and 4)
calib_jet_pt_cols  = ["jet1_pt",  "jet2_pt", "jet3_pt", "jet4_pt"]
calib_jet_eta_cols = ["jet1_eta", "jet2_eta", "jet3_eta", "jet4_eta"]
calib_jet_tag_cols = ["jet1_btagPNetQvG", "jet2_btagPNetQvG","jet3_btagPNetQvG", "jet4_btagPNetQvG"]

N_PT_BINS_CENTRAL = 8
N_PT_BINS_FORWARD = 4
N_TAG_BINS        = 20

tag_edges   = np.linspace(0.0, 1.0, N_TAG_BINS + 1)
tag_centres = 0.5 * (tag_edges[:-1] + tag_edges[1:])


def collect_jets_inclusive(sub_df, pt_cols, eta_cols, tag_cols):
    """Stack (pt, eta, tag, weight) over the given jet columns."""
...
pt4_mc,   eta4_mc,   tag4_mc,   wgt4_mc   = collect_jets_inclusive(
    df_mc,   calib_jet_pt_cols, calib_jet_eta_cols, calib_jet_tag_cols)
pt4_data, eta4_data, tag4_data, wgt4_data = collect_jets_inclusive(
    df_data, calib_jet_pt_cols, calib_jet_eta_cols, calib_jet_tag_cols)

abs_eta4_data = np.abs(eta4_data)
abs_eta4_mc   = np.abs(eta4_mc)

# ── pt edges per region (quantile-based from data) ────────────────────────────
pt_edges_central = weighted_quantile_edges(
    pt4_data[abs_eta4_data <= 2.5], wgt4_data[abs_eta4_data <= 2.5],
...
# ── eta edges: quantile-based with fixed boundaries at 2.5 and 3.0 ───────────
N_ETA_BINS_BARREL  = 8
N_ETA_BINS_TRANS   = 2
N_ETA_BINS_FORWARD = 3

eta_edges_barrel  = weighted_quantile_edges(
    abs_eta4_data, wgt4_data, N_ETA_BINS_BARREL, 0.0, 2.5)
eta_edges_trans   = weighted_quantile_edges(
    abs_eta4_data, wgt4_data, N_ETA_BINS_TRANS,  2.5, 3.0)
eta_edges_fwd_reg = weighted_quantile_edges(
    abs_eta4_data, wgt4_data, N_ETA_BINS_FORWARD, 3.0, 4.7)
...
# split eta edges for central/forward regions
eta_edges_central = eta_edges[eta_edges <= 2.5 + 1e-9]
eta_edges_forward = eta_edges[eta_edges >= 2.5 - 1e-9]

pt_centres_central = 0.5 * (pt_edges_central[:-1] + pt_edges_central[1:])
pt_centres_forward = 0.5 * (pt_edges_forward[:-1] + pt_edges_forward[1:])
eta_centres        = 0.5 * (eta_edges[:-1]         + eta_edges[1:])

# ── region selectors ──────────────────────────────────────────────────────────
def get_central(pt_v, eta_v, tag_v, wgt_v):
    m = (np.abs(eta_v) <= 2.5) & (pt_v >= 30.0) & (pt_v <= 450.0)
    return pt_v[m], np.abs(eta_v[m]), tag_v[m], wgt_v[m]

def get_forward(pt_v, eta_v, tag_v, wgt_v):
    m = (np.abs(eta_v) > 2.5) & (np.abs(eta_v) <= 4.7) & (pt_v >= 50.0) & (pt_v <= 450.0)
    return pt_v[m], np.abs(eta_v[m]), tag_v[m], wgt_v[m]


def make_3d_hist_region(pt_vals, eta_vals, tag_vals, weights, pt_edges_r, eta_edges_r):
    h, _ = np.histogramdd(
        np.column_stack([pt_vals, eta_vals, tag_vals]),
...
def make_3d_cdf(h3d):
    cdf       = np.zeros((*h3d.shape[:2], h3d.shape[2] + 1), dtype=float)
    cumsum    = np.cumsum(h3d, axis=2)
    row_total = cumsum[:, :, -1:].copy()
    row_total[row_total == 0] = 1
    cdf[:, :, 1:] = cumsum / row_total
    return cdf


h3d_mc_cen   = make_3d_hist_region(*get_central(pt4_mc,   eta4_mc,   tag4_mc,   wgt4_mc),
                                    pt_edges_central, eta_edges_central)
h3d_data_cen = make_3d_hist_region(*get_central(pt4_data, eta4_data, tag4_data, wgt4_data),
                                    pt_edges_central, eta_edges_central)
h3d_mc_fwd   = make_3d_hist_region(*get_forward(pt4_mc,   eta4_mc,   tag4_mc,   wgt4_mc),
                                    pt_edges_forward, eta_edges_forward)
h3d_data_fwd = make_3d_hist_region(*get_forward(pt4_data, eta4_data, tag4_data, wgt4_data),
                                    pt_edges_forward, eta_edges_forward)

cdf_mc_cen   = make_3d_cdf(h3d_mc_cen)
cdf_data_cen = make_3d_cdf(h3d_data_cen)
cdf_mc_fwd   = make_3d_cdf(h3d_mc_fwd)
cdf_data_fwd = make_3d_cdf(h3d_data_fwd)


def calibrate_tag_3d(pt_vals, eta_vals, tag_vals):
    ...
    if in_cen.any():
        ...
        tv = tag_vals[in_cen]; cal_in = tv.copy()
        for i in np.unique(pt_idx):
            for j in np.unique(eta_idx[pt_idx == i]):
                mask = (pt_idx == i) & (eta_idx == j)
                if not mask.any():
                    continue
                quantiles    = np.interp(tv[mask], tag_edges, cdf_mc_cen[i, j])
                _, uidx      = np.unique(cdf_data_cen[i, j], return_index=True)
                cal_in[mask] = np.interp(quantiles,
                                         cdf_data_cen[i, j][uidx],
                                         tag_edges[uidx])
        cal_vals[in_cen] = cal_in
    ...
    if in_fwd.any():
        ...
        tv = tag_vals[in_fwd]; cal_in = tv.copy()
        for i in np.unique(pt_idx):
            for j in np.unique(eta_idx[pt_idx == i]):
                mask = (pt_idx == i) & (eta_idx == j)
                if not mask.any():
                    continue
                quantiles    = np.interp(tv[mask], tag_edges, cdf_mc_fwd[i, j])
                _, uidx      = np.unique(cdf_data_fwd[i, j], return_index=True)
                cal_in[mask] = np.interp(quantiles,
                                         cdf_data_fwd[i, j][uidx],
                                         tag_edges[uidx])
        cal_vals[in_fwd] = cal_in

    return cal_vals


# ── save calibration script ───────────────────────────────────────────────────
calib_code = f"""# Auto-generated 3D btagPNetQvG calibration
# Derived from jet1 + jet2 + jet3 + jet4 stacked (inclusive)
# Central region: |eta| <= 2.5, pt in [30, 450] GeV
# Forward region: |eta| in (2.5, 4.7], pt in [50, 450] GeV
# Jets outside these ranges are left unchanged.
...
cdf_mc_cen   = np.array({cdf_mc_cen.tolist()})
cdf_data_cen = np.array({cdf_data_cen.tolist()})
cdf_mc_fwd   = np.array({cdf_mc_fwd.tolist()})
cdf_data_fwd = np.array({cdf_data_fwd.tolist()})

def calibrate_btagPNetQvG(pt_vals, eta_vals, tag_vals):
    \"\"\"
    Calibrate jet btagPNetQvG. Jets outside calibration range are left unchanged.
    \"\"\"
    pt_vals  = np.asarray(pt_vals,  dtype=float)
    abs_eta  = np.abs(np.asarray(eta_vals, dtype=float))
    tag_vals = np.asarray(tag_vals, dtype=float)
    ...
    if in_cen.any():
        ...
        for i in np.unique(pt_idx):
            for j in np.unique(eta_idx[pt_idx == i]):
                ...
                quantiles    = np.interp(tv[mask], tag_edges, cdf_mc_cen[i, j])
                _, uidx      = np.unique(cdf_data_cen[i, j], return_index=True)
                cal_in[mask] = np.interp(quantiles, cdf_data_cen[i, j][uidx],
                                         tag_edges[uidx])
        cal_vals[in_cen] = cal_in
    ...
    if in_fwd.any():
        ...
        for i in np.unique(pt_idx):
            for j in np.unique(eta_idx[pt_idx == i]):
                ...
                quantiles    = np.interp(tv[mask], tag_edges, cdf_mc_fwd[i, j])
                _, uidx      = np.unique(cdf_data_fwd[i, j], return_index=True)
                cal_in[mask] = np.interp(quantiles, cdf_data_fwd[i, j][uidx],
                                         tag_edges[uidx])
        cal_vals[in_fwd] = cal_in
...
# ── apply calibration to ALL jets in MC ──────────────────────────────────────
...
for pt_col, eta_col, tag_col in zip(jet_pt_cols, jet_eta_cols, jet_tag_cols):
    ...
    df_mc_cal.loc[valid, tag_col] = calibrate_tag_3d(
...
# ─────────────────────────────────────────────────────────────────────────────
# Diagnostic plots: PDF, CDF slices
# ─────────────────────────────────────────────────────────────────────────────

# marginalised 2D PDF (eta-integrated) for central region
h2d_mc_marg   = h3d_mc_cen.sum(axis=1)
h2d_data_marg = h3d_data_cen.sum(axis=1)

def pt_normalised_pdf(h2d):
    pdf     = h2d.copy().astype(float)
    row_sum = pdf.sum(axis=1, keepdims=True) * np.diff(tag_edges)
    row_sum[row_sum == 0] = 1
    return pdf / row_sum

pdf_mc_marg   = pt_normalised_pdf(h2d_mc_marg)
pdf_data_marg = pt_normalised_pdf(h2d_data_marg)

extent_2d_cen = [tag_edges[0], tag_edges[-1],
                 pt_edges_central[0], pt_edges_central[-1]]

for label, pdf in [("Data", pdf_data_marg), ("MC", pdf_mc_marg)]:
    fig, ax = plt.subplots(figsize=(7, 6))
    im = ax.imshow(pdf, aspect="auto", origin="lower",
                   extent=extent_2d_cen, cmap="Blues")
    plt.colorbar(im, ax=ax, label="PDF (per pt slice, eta-integrated)")
    ax.set_xlabel("jet_btagPNetQvG"); ax.set_ylabel("jet pt  [GeV]")
    ax.set_title(f"jet1+jet2+jet3+jet4 btagPNetQvG vs pt (central, eta-integrated) — {label}")
    plt.tight_layout()
    fig.savefig(os.path.join(config["output"], f"btagPNetQvG_pt_pdf_{label}.png"),
                dpi=150, bbox_inches="tight")
    plt.close(fig)

with np.errstate(divide="ignore", invalid="ignore"):
    pdf_ratio_marg = np.where(pdf_mc_marg > 0,
                              pdf_data_marg / pdf_mc_marg, np.nan)

finite = pdf_ratio_marg[np.isfinite(pdf_ratio_marg)]
vmin   = np.nanpercentile(finite, 2); vmax = np.nanpercentile(finite, 98)
vmin   = min(vmin, 2.0 - vmax);      vmax = max(vmax, 2.0 - vmin)
norm_r = TwoSlopeNorm(vmin=vmin, vcenter=1.0, vmax=vmax)
cmap_r = plt.cm.RdBu_r.copy(); cmap_r.set_bad("lightgrey")

fig, ax = plt.subplots(figsize=(7, 6))
im = ax.imshow(pdf_ratio_marg, aspect="auto", origin="lower",
               extent=extent_2d_cen, cmap=cmap_r, norm=norm_r)
plt.colorbar(im, ax=ax, label="Data / MC PDF")
ax.set_xlabel("jet_btagPNetQvG"); ax.set_ylabel("jet pt  [GeV]")
ax.set_title("jet1+jet2+jet3+jet4 btagPNetQvG vs pt (central, eta-integrated) — Data/MC ratio")
plt.tight_layout()
fig.savefig(os.path.join(config["output"], "btagPNetQvG_pt_pdf_ratio.png"),
            dpi=150, bbox_inches="tight")
plt.close(fig)

# CDF slices (central region)
fig, ax = plt.subplots(figsize=(7, 5))
pt_slice_idxs  = [1, 4, 7]
eta_slice_idxs = [1, int(len(eta_edges_central) // 2), len(eta_edges_central) - 3]
colors_cdf     = plt.cm.viridis(np.linspace(0, 1,
                                len(pt_slice_idxs) * len(eta_slice_idxs)))
c = 0
for pi in pt_slice_idxs:
    for ei in eta_slice_idxs:
        if pi >= cdf_data_cen.shape[0] or ei >= cdf_data_cen.shape[1]:
            continue
        lbl = (f"pt∈[{pt_edges_central[pi]:.0f},{pt_edges_central[pi+1]:.0f}] "
               f"η∈[{eta_edges_central[ei]:.1f},{eta_edges_central[ei+1]:.1f}]")
        ax.plot(tag_edges, cdf_data_cen[pi, ei],
                color=colors_cdf[c], linewidth=1.5, label=lbl)
        ax.plot(tag_edges, cdf_mc_cen[pi, ei],
                color=colors_cdf[c], linewidth=1.5, linestyle="--")
        c += 1
ax.plot([], [], color="grey", linewidth=1.5,                label="Data")
ax.plot([], [], color="grey", linewidth=1.5, linestyle="--", label="MC")
ax.set_xlabel("jet_btagPNetQvG"); ax.set_ylabel("CDF")
ax.set_title("jet1+jet2+jet3+jet4 btagPNetQvG CDF per (pt, η) cell — central")
ax.legend(fontsize=6, ncol=2)
plt.tight_layout()
fig.savefig(os.path.join(config["output"], "btagPNetQvG_pt_eta_cdf_slices.png"),
            dpi=150, bbox_inches="tight")
plt.close(fig)
